...
| 2026-09-21 | `suno`      | `suno` | `studio-api.suno.ai`, `studio-api-prod.suno.com` | Project owner decision (`#14224`)    | [#14432](https://github.com/diegosouzapw/OmniRoute/pull/14432) | Cookie-auth provider, not an operator takedown. `studio-api.suno.ai` was suspended (503); the live host requires a Clerk JWT exchange plus a paid hCaptcha-solving dependency that OmniRoute chose not to add for one music provider. `kie/suno-v4.0` and `kie/suno-v3.5` (kie.ai-hosted) are unaffected and remain supported. |
